	...
	public void run(int ms) {
		...
		if (w)
			left.setCenter(left.getCenter().x,
					left.getCenter().y - (left.getVelocity().y * (ms / 100f)));
		if (s)
			left.setCenter(left.getCenter().x,
					left.getCenter().y + (left.getVelocity().y * (ms / 100f)));

		if (up)
			right.setCenter(right.getCenter().x,
					right.getCenter().y - (right.getVelocity().y * (ms / 100f)));
		if (down)
			right.setCenter(right.getCenter().x,
					right.getCenter().y + (right.getVelocity().y * (ms / 100f)));
		...
		if (ball.isDead()) {
			...
			ball = new Ball(engine, ballImage);
			ball.setCenter(
					(engine.getEnvironmentSize().x / 2)
							- (ball.getSize().x / 2),
					(engine.getEnvironmentSize().y / 2)
							- (ball.getSize().y / 2));
			engine.actors.add(ball);
		}
	}
